...
const layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="bg-white p-3 md:p-5 min-h-screen">
      ...
      {openContractPopUp && <NewContract setOpenContractPopUp={toggleContractPopUp} />}

      {contractToEdit && isEditingEnabled && (
        <EditContract cancelEdit={removeEditingMode} contract={contractToEdit} />
      )}

      {/* Search, filter, and add new contract button */}
      <div className="flex justify-between gap-x-4 items-center mt-10">
        <div className="flex gap-x-5 justify-between">
          ...
          <div className="flex justify-start gap-x-3 md:gap-x-0 md:justify-between w-[220px] md:w-[359px] items-center p-2 md:p-4 border-gray-500 h-10 md:h-14 rounded-2xl border-[0.48px]">
            ...
            <input
              className="w-40 md:w-60 placeholder:text-black placeholder:font-light placeholder:font-outfit font-outfit placeholder:text-sm text-black font-light text-xs md:text-sm outline-none"
              type="text"
              placeholder="Search by name, role, department..."
            />
            <div className="md:flex hidden justify-center items-center gap-x-[2px]">
        ...
        <button
          ...
          className="flex justify-between gap-x-2 bg-[#DDFF8F] items-center md:px-4 px-6 py-2 md:py-4 h-10 md:h-14 rounded-2xl"
        >
          <p className="text-black font-outfit font-light hidden md:block text-sm capitalize">
            Add new Contract
          </p>
